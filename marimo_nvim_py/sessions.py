...
def _cell_config(cell: dict[str, Any]) -> CellConfig:
    ...
    return CellConfig.from_dict(options if isinstance(options, dict) else {}, warn=False)


def _cell_options(cell: dict[str, Any]) -> dict[str, Any]:
    options = cell.get("options") or {}
...
def _cell_disabled_states(cells: list[dict[str, Any]]) -> dict[str, bool]:
    ...
    for cell in cells:
        ...
        disabled_by_ancestor[cell["id"]] = bool(disabled and not _cell_options(cell).get("disabled"))
...
def _cell_disabled_map(cells: list[dict[str, Any]]) -> dict[str, bool]:
    ...
    try:
        ...
        return {
            cell["id"]: bool(_cell_options(cell).get("disabled"))
            for cell in cells
        }
...
class Worker:
    ...
    def _update_session_from_payload(self, session: Session, payload: dict[str, Any]) -> None:
        session.header = payload.get("header")
        app_options = payload.get("app_options") or {}
        session.app_options = dict(app_options) if isinstance(app_options, dict) else {}
        session.cells = [dict(cell) for cell in payload.get("cells") or []]

    def _cell_changes(
        self, previous_cells: list[dict[str, Any]], current_cells: list[dict[str, Any]]
        ...
        for cell in current_cells:
            ...
            if (
                ...
                or _cell_options(previous) != _cell_options(cell)
    ...
    def _runtime_payload(self, session: Session) -> dict[str, Any]:
    ...
    def _refresh_runtime_payload(self, session: Session) -> dict[str, Any]:
        self._refresh_runtime_cells(session)
        return self._runtime_payload(session)
    ...
    def ensure_runtime_session(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if session.runtime_session is not None:
            return self._refresh_runtime_payload(session)
    ...
    def _execution_closure(self, session: Session, requested_ids: list[str]) -> list[str]:
        ...
        try:
            ...
            return [
                cell["id"]
                for cell in session.cells
                if cell["id"] in requested and not _cell_options(cell).get("disabled")
            ]
    ...
    def sync_and_run(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        previous_cells = list(session.cells)
        self._update_session_from_payload(session, payload)
        changed_ids, deleted_ids = self._cell_changes(previous_cells, session.cells)
        if not session.runtime_bootstrapped:
            changed_ids = [cell["id"] for cell in session.cells]
        disabled_map = _cell_disabled_map(session.cells)
    ...
    def open_session(self, params: dict[str, Any]) -> dict[str, Any]:
        path = params["path"]
        payload = params["payload"]
        session_id = params.get("session_id", path)
        ...
        session = Session(
            ...
            app_options=dict(payload["app_options"]) if isinstance(payload.get("app_options"), dict) else {},
            cells=[dict(cell) for cell in payload.get("cells") or []],
            runtime_cells={},
    ...
    def write_notebook(self, params: dict[str, Any]) -> dict[str, Any]:
        path = params["path"]
        canonical_source = params.get("canonical_source")
        if not isinstance(canonical_source, str):
            serialized = serialize_notebook(path, params["snapshot"])
            canonical_source = serialized["canonical_source"]
        Path(path).write_text(canonical_source, encoding="utf-8")
        return {
            "written": True,
            "canonical_source": canonical_source,
            "last_saved_source_hash": hashlib.sha256(canonical_source.encode("utf-8")).hexdigest(),
        }

    def reload_from_disk(self, params: dict[str, Any]) -> dict[str, Any]:
        session = self.sessions[params["session_id"]]
        content = Path(session.path).read_text(encoding="utf-8")
        payload = load_raw_notebook(session.path, content)
        serialized = serialize_notebook(session.path, payload)
        payload["canonical_source"] = serialized["canonical_source"]
        return self.open_session({"path": session.path, "payload": payload, "project_root": session.project_root, "runtime_kind": session.runtime_kind})
    ...
    def get_canonical_source(self, params: dict[str, Any]) -> dict[str, Any]:
        raise KeyError("canonical source is Lua-owned")

    def get_projection_map(self, params: dict[str, Any]) -> dict[str, Any]:
        raise KeyError("projection map is Lua-owned")
