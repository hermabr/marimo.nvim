...
local function test_mode_toggle_keymap_callback_works()
	...
	assert_eq(toggle_map.rhs, "<Cmd>Marimo<CR>")
	assert_truthy(not vim.b.marimo_projected)

	local toggle_keys = vim.api.nvim_replace_termcodes("<leader>mm", true, false, true)
	vim.api.nvim_feedkeys(toggle_keys, "xt", false)
	wait_for_truthy(function()
		return vim.b.marimo_projected
	end, "timed out waiting for <leader>mm to activate marimo")
	...
	vim.api.nvim_feedkeys(toggle_keys, "xt", false)
	wait_for_truthy(function()
		return not vim.b.marimo_projected
	end, "timed out waiting for <leader>mm to deactivate marimo")
	assert_eq(vim.api.nvim_buf_get_lines(0, 0, -1, false)[1], "x = 1")
	...
	wait_for_truthy(function()
		return vim.b.marimo_projected
	end, "timed out waiting for <leader>mm to reactivate marimo")
	assert_eq(vim.api.nvim_buf_get_lines(0, 0, 1, false)[1], "# + {marimo}")
